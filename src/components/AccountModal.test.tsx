...
const FREE: SubscriptionSummary = { isPro: false, isCompAccount: false, plan: 'none', currency: 'none', currentPeriodEnd: null }
...
describe('AccountModal plan display', () => {
  ...
  it('shows the plan, renewal date, and a Manage billing link for a Pro subscriber', async () => {
    ...
    await renderAccountModal({ isPro: true, isCompAccount: false, plan: 'quarterly', currency: 'eur', currentPeriodEnd: future })
  ...
  it('shows "Unlimited (team account)" for a comp account, with no billing link at all', async () => {
    await renderAccountModal({ isPro: true, isCompAccount: true, plan: 'none', currency: 'none', currentPeriodEnd: null })
  ...
  it('redirects to the returned Stripe Portal URL when Manage billing succeeds', async () => {
    ...
    await renderAccountModal(
      { isPro: true, isCompAccount: false, plan: 'monthly', currency: 'usd', currentPeriodEnd: '2026-08-01T00:00:00.000Z' },
