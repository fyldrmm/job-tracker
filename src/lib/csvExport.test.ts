...
describe('buildApplicationsCsv', () => {
  it('returns just the header row for an empty list', () => {
    const csv = buildApplicationsCsv([], trackers)
    expect(csv).toBe(
      'Company,Role,Tracker,Stage,Date applied,Salary range,Location,Employment type,Work mode,Priority,Archived,Archive reason,Archived at,Job link,Notes',
    )
  })
  ...
  it('renders a full row with resolved labels', () => {
    ...
    const csv = buildApplicationsCsv([app], trackers)
    const [, row] = csv.split('\n')
    expect(row).toBe(
      'Acme,Engineer,Grad roles,Interview,2026-02-01,100k-120k,Remote,Full-time,Remote,Yes,Yes,Rejected,2026-03-01T00:00:00.000Z,https://example.com/job,Great fit',
    )
  })
  ...
  it('renders empty cells for null optional fields', () => {
    ...
    const csv = buildApplicationsCsv([app], trackers)
    const [, row] = csv.split('\n')
    expect(row).toBe('Acme,Engineer,Senior roles,Applied,2026-01-01,,,,,No,No,,,,')
  })
