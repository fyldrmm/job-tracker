...
- **Active milestone:** none — M8 (AI job extraction) is fully built and live, plus its follow-ons (employment/work-mode Archive filters, `internship` type addition, employment/work-mode shown on archive rows) and a newly-found gap fix: permanently deleting an application (initially archive-only, then extended to active/board applications too after user feedback that requiring archive-first was too much friction). All committed, pushed, and verified — including live signed-in QA confirming the Supabase delete + `stage_history` cascade.
- **Next action:** nothing blocking. Everything raised so far is built, verified, and live.
