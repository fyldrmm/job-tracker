...
- **Last completed:** multiple named trackers ("tabs") per user, now fully closed out — user confirmed sign-out/sign-in no longer leaks or loses data, and a 4th bug (duplicate "My Applications" trackers auto-created on every fresh guest visit) was found and fixed: trackers are no longer auto-created at all — the user now gets an explicit "Create your first tracker" prompt on true zero-tracker state (guest or new account). See bug list below.
...
- **Next action:** nothing blocking — the project is in a stable, feature-complete state. Remaining items are the user's call: hosting/launch, cleaning up stray test users in Supabase Auth (the old `resend.dev`-era typo'd/test accounts), or new feature requests. See "Postponed / deferred" section below for the full list.
...
- **Deletion-confirmation email.** When "Delete account" user requested this (send an email confirming the account was deleted), but explicitly agreed to defer it. Needs a Supabase Edge Function (new server-side infra for this project), a Resend API key kept server-side (never in frontend code), and a Supabase CLI personal access token to deploy the function (see the M1 note on where to find that token). Not started.
- **Hosting decision.** Never actually decided — was marked "TBD, decide before M6/launch" since M1 and still is. Needed before any real public launch.
- **Stray/test Supabase Auth users.** A few leftover test accounts exist in Authentication → Users from earlier milestones (a typo'd `fyildirimo2012@gmail.com`, possibly an older `mailinator.com` test account from M5). Not a GDPR issue (confirmed earlier — these are the developer's own test signups, not real user data), just housekeeping. Delete manually from the Supabase dashboard whenever convenient.
- **Show which tracker an archived application belongs to, inside `CardDetail.tsx`.** Minor nice-to-have noted verbally during the multi-tracker build — useful since the Archive view is shared across all trackers, so opening a card from Archive doesn't currently say which tracker it came from (only the Archive list's grouping shows that). Small, self-contained change whenever it's wanted.
- **Real Ko-fi/donation flow verification.** The Ko-fi link (`https://ko-fi.com/fazare`) is wired up in the header and sidebar, but nobody's actually clicked through the full external Ko-fi donation flow to confirm it looks right from a visitor's perspective — not urgent, just untested end-to-end.
