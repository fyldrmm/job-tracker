...
- **Next action:** none outstanding. `AUDIT.md` is fully closed (all of B, C, D), the M6-regression bug it surfaced is fixed, and C6 (extraction discoverability) is built, live-verified, and redeployed (the Edge Function's quota-message wording, `d415b34`, needed and got a manual `extract-job-details` redeploy — user confirmed done). Working tree clean, nothing uncommitted, no stash. Pick the next milestone.
