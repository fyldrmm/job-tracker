...
- **M9 — Follow-up reminders (push only, no email) — done, needs live QA.** User picked this from the 3 candidates below, scoped to: auto-trigger on time-since-stage-change (reuses the existing 14-day stale threshold, now shared via `src/lib/stale.ts`), delivery via the browser `Notification` API while the tab is open/backgrounded (no service worker/VAPID/true closed-browser push), and a `localStorage`-backed on/off toggle (bell icon) in `Sidebar.tsx` that requests OS permission on click. `src/hooks/useStaleReminders.ts` polls every 5 min + on mount, notifies once per app per stage-change (dedup via `src/lib/reminders.ts`'s notified-map), batches multiple due apps into one notification, skips archived apps. 70 tests passing (9 new), `tsc`/`oxlint` clean. **Not yet done:** real end-to-end QA in an actual desktop browser (permission prompt → real OS notification) — this environment's preview browser reports `Notification.permission: 'denied'` by default, so only the UI wiring (toggle states, permission-request call) was verified live; the actual notification firing was verified via mocked unit tests only.
