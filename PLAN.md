...
- **Next action:** mid-way through 4 user-proposed features (see "Four user-proposed features" below). (4) rename trackers and (3) right-click context menu are shipped. (1) manual "most wanted" priority flag + accent color just shipped (`81e51ed`) — **needs `supabase/migrations/0009_priority.sql` run in the Supabase dashboard before it works for signed-in users** (guest/IndexedDB works already, no migration needed there). Next up: (2) the browser extension (its own multi-session milestone).
...
- [ ] **(2) Browser extension for job-detail extraction** — not started.

#### Manual step still needed for (1)

1. Run `supabase/migrations/0009_priority.sql` in the Supabase SQL editor. Until this happens, signed-in reads/writes of `is_priority` will fail (column doesn't exist remotely) — guest mode is unaffected since IndexedDB has no fixed schema.
